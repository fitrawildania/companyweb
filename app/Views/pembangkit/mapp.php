...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">MAPP POWER</h1>
                    <a href="mapp/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv1" class="btn btn-primary mb-3">
        <i class="fas fa-solid fa-file-csv"></i> CSV</a>
                    ...
                    <table class="table table-bordered" id="mappTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($mapp_table) && is_iterable($mapp_table)):?>
                                <?php foreach ($mapp_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('mapp/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('mapp/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('mapp/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('mapp/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasimapp"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
...
<script>
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser = [];
            if (is_iterable($mapp_table)) {
                foreach ($mapp_table as $row) {
            ...
            updateChart('servermapp', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasimapp', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanmapp',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupmapp', labels, null, null,null,null,null, null, null, dataBackup);
        });
