...
<?= $this->section('content'); ?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">GBMO GAS</h1>
                    <a href="gas/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv5" class="btn btn-primary mb-3">
        <i class="fas fa-solid fa-file-csv"></i> CSV</a>
                    ...
                    <table class="table table-bordered" id="gasTable">
                        ...
                        <tbody>
                            <?php if(isset($gas_table) && is_iterable($gas_table)):?>
                                <?php foreach ($gas_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('gas/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('gas/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('gas/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('gas/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
...
<script src="https://cdn.datatables.net/1.13.4/js/dataTables.bootstrap5.min.js"></script>
    ...
    <?php 
            $tagihanUser = [];
            if (is_iterable($gas_table)) {
                foreach ($gas_table as $row) {
    ...
    updateChart('servergas', labels, dataAverage, dataMax, null, null, null, null, null, null);
    updateChart('utilisasigas', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
    updateChart('tagihangas', labels, null, null, null, null, null, dataSistem, dataUser, null);
    updateChart('backupgas', labels, null, null, null, null, null, null, null, dataBackup);
});
