...
<?= $this->section('content');?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">BBO</h1>
                    <a href="bbo/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv2" class="btn btn-primary mb-3">
        <i class="fas fa-solid fa-file-csv"></i> CSV</a>
                    ...
                    <table class="table table-bordered" id="bboTable">
                        <thead>
                        <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($bbo_table) && is_iterable($bbo_table)):?>
                                <?php foreach ($bbo_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('bbo/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('bbo/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('bbo/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('bbo/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
    ...
    const rows = document.querySelectorAll('#bboTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser = [];
            if (is_iterable($bbo_table)) {
                foreach ($bbo_table as $row) {
            ...
            updateChart('serverbbo', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasibbo', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanbbo',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupbbo', labels, null, null,null,null,null, null, null, dataBackup);
        });
