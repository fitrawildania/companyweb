...
<?= $this->section('content');?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">SIIPP</h1>
                    <a href="siipp/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv6" class="btn btn-primary mb-3">
        <i class="fas fa-solid fa-file-csv"></i> CSV</a>
                    ...
                    <table class="table table-bordered" id="siippTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($siipp_table) && is_iterable($siipp_table)):?>
                                <?php foreach ($siipp_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('siipp/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('siipp/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('siipp/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('siipp/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasisiipp"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
...
<script>
    ...
    const rows = document.querySelectorAll('#siippTable tbody tr');
    ...
    <?php 
            $tagihanUser = [];
            if (is_iterable($siipp_table)) {
                foreach ($siipp_table as $row) {
    ...
    updateChart('serversiipp', labels, dataAverage, dataMax, null, null, null, null, null, null);
    updateChart('utilisasisiipp', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
    updateChart('tagihansiipp', labels, null, null, null, null, null, dataSistem, dataUser, null);
    updateChart('backupsiipp', labels, null, null, null, null, null, null, null, dataBackup);
});
