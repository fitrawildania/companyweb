...
<div class="container-fluid">
    <h1 class="h3 mt-4">EAM Aset Properti</h1>

    <!-- Form untuk Prediksi -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Prediksi Penggunaan CPU, RAM, dan HDD</h6>
        </div>
        <div class="card-body">
            <form action="<?= site_url('prediction/predict') ?>" method="post">
                <div class="form-group">
                    <label for="periode">Periode (Bulan ke-):</label>
                    <input type="number" class="form-control" id="periode" name="periode" required min="1" max="24">
                </div>
    ...
    <a href="aset/add" class="btn btn-primary mb-3">
    ...
    <a href="/csv7" class="btn btn-primary mb-3">
    ...
    <div id="filteredTable">
        <table class="table table-bordered" id="asetTable">
            <thead>
            <tr>
                <th>Date</th>
                <th>Periode</th>
            ...
            <tbody>
                <?php if(isset($aset_table) && is_iterable($aset_table)):?>
                    <?php foreach ($aset_table as $row): ?>
                    <tr>
                    <td><?= $row['date'] ?></td>
                    <td><?= $row['periode'] ?></td>
                    <!-- <td><?= $row['ip'] ?></td> -->
                    ...
                    <td><?= $row['ram'] ?> %</td>
                    <td><?= $row['hdd'] ?> %</td>
                    <td><?= number_format($row['tagihan_sistem'], 0, ',', '.') ?></td>
                    <td><?= number_format($row['tagihan_user'], 0, ',', '.') ?></td>
                    <td><?= $row['backup'] ?> GB</td>
                    <td>
                        <a href="<?= site_url('aset/view/' . basename($row['file_path'])) ?>"
                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                        <a href="<?= site_url('aset/download/' . basename($row['file_path'])) ?>"
                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                    </td>                
                    <td>
                        <a href="<?= site_url('aset/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                        <a href="<?= site_url('aset/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                    </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
        ...
        <div class="row row-cols-auto">
            ...
            <div class="col">
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <canvas id="utilisasiaset"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    <?php 
            $tagihanUser = [];
            if (is_iterable($aset_table)) {
                foreach ($aset_table as $row) {
    ...
    updateChart('serveraset', labels, dataAverage, dataMax, null, null, null, null, null, null);
    updateChart('utilisasiaset', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
    updateChart('tagihanaset', labels, null, null, null, null, null, dataSistem, dataUser, null);
    updateChart('backupaset', labels, null, null, null, null, null, null, null, dataBackup);
});
