...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">EAM-KIT</h1>
                    <a href="maximo/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv3" class="btn btn-primary mb-3">
        <i class="fas fa-solid fa-file-csv"></i> CSV</a>
                    ...
                    <table class="table table-bordered" id="maximoTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($maximo_table) && is_iterable($maximo_table)):?>
                                <?php foreach ($maximo_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('maximo/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('maximo/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('maximo/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('maximo/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasimaximo"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="tagihanmaximo"></canvas>
                        </div>
                    </div>
                    </div>
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    ...
    const rows = document.querySelectorAll('#maximoTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser = [];
            if (is_iterable($maximo_table)) {
                foreach ($maximo_table as $row) {
            ...
            updateChart('servermaximo', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasimaximo', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanmaximo',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupmaximo', labels, null, null,null,null,null, null, null, dataBackup);
        });
    function updateChart(chartId, labels, dataAverage, dataMax, dataCPU, dataRAM, dataHDD, dataSistem, dataUser, dataBackup) {
        ...
        // Cek apakah window[chartId] adalah instance dari Chart dan bukan undefined
