...
<?= $this->section('content');?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">GBMO BBM</h1>
                    <a href="bbm/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv4" class="btn btn-primary mb-3">
        <i class="fas fa-solid fa-file-csv"></i> CSV</a>
                    ...
                    <table class="table table-bordered" id="bbmTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($bbm_table) && is_iterable($bbm_table)):?>
                                <?php foreach ($bbm_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('bbm/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('bbm/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('bbm/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('bbm/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
...
$(document).ready(function() {
    ...
    const rows = document.querySelectorAll('#bbmTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser =[];
            if (is_iterable($bbm_table)){
                foreach ($bbm_table as $row) {
            ...
            updateChart('serverbbm', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasibbm', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanbbm',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupbbm', labels, null, null,null,null,null, null, null, dataBackup);
        });
