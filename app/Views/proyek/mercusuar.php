...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">Mercusuar</h1>
                    <a href="eic/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv15" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="mercusuarTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($mercusuar_table) && is_iterable($mercusuar_table)):?>
                                <?php foreach ($mercusuar_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('mercusuar/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('mercusuar/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('mercusuar/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('mercusuar/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="servermercusuar"></canvas>
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasimercusuar"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="tagihanmercusuar"></canvas>
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="backupmercusuar"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>              
    ...
    const rows = document.querySelectorAll('#mercusuarTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($mercusuar_table)) {
                foreach ($mercusuar_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $mercusuar_table)){
                foreach ($mercusuar_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('servermercusuar', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasimercusuar', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanmercusuar',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupmercusuar', labels, null, null,null,null,null, null, null, dataBackup);
        });
