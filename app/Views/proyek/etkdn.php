...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">ETKDN</h1>
                    <a href="etkdn/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv19" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="etkdnTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($etkdn_table) && is_iterable($etkdn_table)):?>
                                <?php foreach ($etkdn_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('etkdn/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('etkdn/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('etkdn/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('etkdn/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasietkdn"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
    ...
    const rows = document.querySelectorAll('#etkdnTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($etkdn_table)) {
                foreach ($etkdn_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $etkdn_table)){
                foreach ($etkdn_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('serveretkdn', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasietkdn', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanetkdn',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupetkdn', labels, null, null,null,null,null, null, null, dataBackup);
        });
