...
<?= $this->section('content');?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">EIQC</h1>
                    <a href="eiqc/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv14" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="eiqcTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($eiqc_table) && is_iterable($eiqc_table)):?>
                                <?php foreach ($eiqc_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('eiqc/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('eiqc/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('eiqc/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('eiqc/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasieiqc"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
    ...
    const rows = document.querySelectorAll('#eiqcTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($eiqc_table)) {
                foreach ($eiqc_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $eiqc_table)){
                foreach ($eiqc_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('servereiqc', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasieiqc', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihaneiqc',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupeiqc', labels, null, null,null,null,null, null, null, dataBackup);
        });
