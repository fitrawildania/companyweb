...
<?= $this->section('content');?>
                <div class="container-fluid">
                    ...
                    <a href="/csv12" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="eicTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($eic_table) && is_iterable($eic_table)):?>
                                <?php foreach ($eic_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('eic/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a> 
                                        <a href="<?= site_url('eic/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('eic/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('eic/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
    ...
    const rows = document.querySelectorAll('#eicTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($eic_table)) {
                foreach ($eic_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $eic_table)){
                foreach ($eic_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('servereic', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasieic', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihaneic',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupeic', labels, null, null,null,null,null, null, null, dataBackup);
        });
