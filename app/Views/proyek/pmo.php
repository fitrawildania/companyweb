...
<?= $this->section('content');?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">PMO</h1>
                    <a href="pmo/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv16" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="pmoTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($pmo_table) && is_iterable($pmo_table)):?>
                            <?php foreach ($pmo_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('pmo/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('pmo/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('pmo/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('pmo/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                                <?php else: ?>
...
<script>
    ...
    const rows = document.querySelectorAll('#pmoTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($pmo_table)) {
                foreach ($pmo_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $pmo_table)){
                foreach ($pmo_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('serverpmo', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasipmo', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanpmo',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backuppmo', labels, null, null,null,null,null, null, null, dataBackup);
        });
