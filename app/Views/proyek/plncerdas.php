...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">PLN Cerdas</h1>
                    <a href="plncerdas/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv18" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="plncerdasTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($plncerdas_table) && is_iterable($plncerdas_table)):?>
                                <?php foreach ($plncerdas_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('plncerdas/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('plncerdas/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('plncerdas/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('plncerdas/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="serverplncerdas"></canvas>
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasiplncerdas"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="tagihanplncerdas"></canvas>
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="backupplncerdas"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>              
...
<script>
    ...
    const rows = document.querySelectorAll('#plncerdasTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($plncerdas_table)) {
                foreach ($plncerdas_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $plncerdas_table)){
                foreach ($plncerdas_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('serverplncerdas', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasiplncerdas', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanplncerdas',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupplncerdas', labels, null, null,null,null,null, null, null, dataBackup);
        });
