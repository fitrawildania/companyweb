...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">EC</h1>
                    <a href="ec/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv13" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="ecTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($ec_table) && is_iterable($ec_table)):?>
                                <?php foreach ($ec_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('ec/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('ec/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('ec/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('ec/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
    ...
    const rows = document.querySelectorAll('#ecTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $ec_table)){
                foreach ($ec_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('serverec', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasiec', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanec',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupec', labels, null, null,null,null,null, null, null, dataBackup);
        });
