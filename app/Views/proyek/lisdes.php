...
<?= $this->section('content');?>
                <div class="container-fluid">
                    <h1 class="h3 mt-4">LISDES</h1>
                    <a href="lisdes/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv20" class="btn btn-primary mb-3">
                    ...
                    <div id="filteredTable"><table class="table table-bordered" id="lisdesTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($lisdes_table) && is_iterable($lisdes_table)):?>
                                <?php foreach ($lisdes_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('lisdes/view/' . basename($row['file_path'])) ?>"class="btn btn-warning btn-sm">
                                            <i class="fas fa-file"></i></a>
                                        <a href="<?= site_url('lisdes/download/' . basename($row['file_path'])) ?>"class="btn btn-primary btn-sm">
                                            <i class="fas fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('lisdes/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('lisdes/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasilisdes"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="tagihanlisdes"></canvas>
                        </div>
                    </div>
                    </div>
...
<script>
    ...
    const rows = document.querySelectorAll('#lisdesTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        <?php 
            $tagihanUser = [];
            if (is_iterable($lisdes_table)) {
                foreach ($lisdes_table as $row) {
            ...
            <?php 
            $tagihanSistem = [];
            if (is_iterable(value: $lisdes_table)){
                foreach ($lisdes_table as $row) {
                    $tagihanSistem[] = $row['tagihan_sistem'];
                }
            }?>

            updateChart('serverlisdes', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasilisdes', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanlisdes',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backuplisdes', labels, null, null,null,null,null, null, null, dataBackup);
        });
