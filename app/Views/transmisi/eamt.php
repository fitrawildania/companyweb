...
<div class="container-fluid">
    <h1 class="h3 mt-4">EAM Transmisi</h1>
    <a href="eamt/add" class="btn btn-primary mb-3">
    ...
    <a href="/csv8" class="btn btn-primary mb-3">
    ...
    <div id="filteredTable">
        <table class="table table-bordered" id="eamtTable">
            <thead>
            <tr>
                <th>Date</th>
                <th>Periode</th>
            ...
            <tbody>
                <?php if(isset($eamt_table) && is_iterable($eamt_table)):?>
                    <?php foreach ($eamt_table as $row): ?>
                    <tr>
                    <td><?= $row['date'] ?></td>
                    <td><?= $row['periode'] ?></td>
                    <td><?= $row['ip'] ?></td>
                    ...
                    <td><?= $row['backup'] ?> GB</td>
                    <td>
                        <a href="<?= site_url('eamt/view/' . basename($row['file_path'])) ?>"
                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                        <a href="<?= site_url('eamt/download/' . basename($row['file_path'])) ?>"
                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                    </td>                
                    <td>
                        <a href="<?= site_url('eamt/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                        <a href="<?= site_url('eamt/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                    </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
        ...
        <div class="row row-cols-auto">
            ...
            <div class="col">
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <canvas id="utilisasieamt"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    const rows = document.querySelectorAll('#eamtTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser = [];
            if (is_iterable($eamt_table)) {
                foreach ($eamt_table as $row) {
            ...
            updateChart('servereamt', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasieamt', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihaneamt',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupeamt', labels, null, null,null,null,null, null, null, dataBackup);
        });
