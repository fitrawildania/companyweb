...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">MM-NE</h1>
                    <a href="mm/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv11" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="mmTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($mm_table) && is_iterable($mm_table)):?>
                            <?php foreach ($mm_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('mm/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('mm/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('mm/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('mm/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                                <?php else: ?>
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    ...
    const rows = document.querySelectorAll('#mmTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser =[];
            if (is_iterable($mm_table)){
                foreach ($mm_table as $row) {
            ...
            updateChart('servermm', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasimm', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanmm',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupmm', labels, null, null,null,null,null, null, null, dataBackup);
        });
