...
<?= $this->section('content');?>
                ...
                <div class="container-fluid">
                    <h1 class="h3 mt-4">Power Inspect</h1>
                    <a href="pi/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv9" class="btn btn-primary mb-3">
                    ...
                    <div id="filteredTable"><table class="table table-bordered" id="piTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($pi_table) && is_iterable($pi_table)):?>
                            <?php foreach ($pi_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('pi/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('pi/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('pi/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('pi/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                                <?php else: ?>
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    ...
    const rows = document.querySelectorAll('#piTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser =[];
            if (is_iterable($pi_table)){
                foreach ($pi_table as $row) {
            ...
            updateChart('serverpi', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasipi', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanpi',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backuppi', labels, null, null,null,null,null, null, null, dataBackup);
        });
