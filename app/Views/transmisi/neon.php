<?= $this->extend('transmisi/layout');?>

<?= $this->section('content');?>


                <div class="container-fluid">
                    <h1 class="h3 mt-4">Neon</h1>
                    <a href="neon/add" class="btn btn-primary mb-3">
                    ...
                    <a href="/csv10" class="btn btn-primary mb-3">
                    ...
                    <table class="table table-bordered" id="neonTable">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Periode</th>
                        ...
                        <tbody>
                        <?php if(isset($neon_table) && is_iterable($neon_table)):?>
                            <?php foreach ($neon_table as $row): ?>
                                <tr>
                                    <td><?= $row['date'] ?></td>
                                    <td><?= $row['periode'] ?></td>
                                    <td><?= $row['ip'] ?></td>
                                    ...
                                    <td><?= $row['backup'] ?> GB</td>
                                    <td>
                                        <a href="<?= site_url('neon/view/' . basename($row['file_path'])) ?>"
                                        class="btn btn-warning btn-sm"><i class="fas fa-light fa-file"></i></a>
                                        <a href="<?= site_url('neon/download/' . basename($row['file_path'])) ?>"
                                        class="btn btn-primary btn-sm"><i class="fas fa-light fa-download"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= site_url('neon/edit/' . $row['id']) ?>" class="btn btn-success btn-sm">Edit</a>
                                        <a href="<?= site_url('neon/delete/' . $row['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <canvas id="utilisasineon"></canvas>
                        </div>
                    </div>
                    </div>
                    </div>
    ...
    const rows = document.querySelectorAll('#neonTable tbody tr');
    ...
    rows.forEach(row => {
        ...
        dataBackup.push(parseFloat(row.cells[10].textContent));
            ...
            <?php 
            $tagihanUser =[];
            if (is_iterable($neon_table)){
                foreach ($neon_table as $row) {
            ...
            updateChart('serverneon', labels, dataAverage, dataMax, null, null, null,null,null,null);
            updateChart('utilisasineon', labels, null, null, dataCPU, dataRAM, dataHDD, null, null, null);
            updateChart('tagihanneon',labels,null, null,null,null,null, dataSistem, dataUser, null);
            updateChart('backupneon', labels, null, null,null,null,null, null, null, dataBackup);
        });
