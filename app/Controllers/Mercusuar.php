...
class Mercusuar extends Controller
{
    public function index($page = 'mercusuar')
    {
        $data = [
            'title' => 'Mercusuar',
            'mercusuar_table' => (new MercusuarModel())-> findAll(),
            'activePage' => $page
        ];

        return view('proyek/mercusuar', $data);
    }

    public function add()
    {
        //ubah
        return view('form_add/mercusuar_add', ['action' => site_url('mercusuar/store'), 'mercusuar' => null]);
    }
    ...
    public function store()
    {
        ...
        $mercusuarModel->insert($mercusuar_table);

        return redirect()->to(site_url('mercusuar'))->with('success', 'Data added successfully');
    }

    public function delete($id)
    {
        $model = new MercusuarModel();
        $model->delete($id);

        // Set flash data and redirect
        session()->setFlashdata('success', 'Data deleted successfully.');
        return redirect()->to('/mercusuar');
    }

    public function edit($id)
    {
        $mercusuarModel = new MercusuarModel();
        $mercusuar = $mercusuarModel->find($id);

        if (!$mercusuar) {
            throw new \CodeIgniter\Exceptions\PageNotFoundException("Asset with ID $id not found");
        }
        //ubah
        return view('form_add/mercusuar_add', ['action' => site_url('mercusuar/update/'.$id), 'mercusuar' => $mercusuar]);
    }
    ...
    public function update($id)
    {
        ...
        if ($filePath) {
            $mercusuar_table['file_path'] = $filePath;
        }

        $mercusuarModel = new MercusuarModel();
        $mercusuarModel->update($id, $mercusuar_table);
    ...
    public function downloadCSV()
    {
        ...
        $dataMercusuar = $mercusuarModel->findAll();

        // Nama file CSV
        $filename = 'mercusuar_export_' . date('Ymd') . '.csv';
